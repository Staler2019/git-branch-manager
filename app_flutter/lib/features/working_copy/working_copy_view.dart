                    ...
                    : ListView(
                        children: <Widget>[
                          if (status.staged.isNotEmpty)
                            _sectionHeader(context, 'STAGED'),
                          for (final entry in status.staged)
                            _rowFor(entry, staged: true),
                          if (status.unstaged.isNotEmpty)
                            _sectionHeader(context, 'UNSTAGED'),
                          for (final entry in status.unstaged)
                            _rowFor(entry, staged: false),
                          if (status.untrackedFiles.isNotEmpty)
                            _sectionHeader(context, 'UNTRACKED'),
                          for (final entry in status.untrackedFiles.where(
                            (e) => !e.hasUnstagedChange,
                          ))
                            _rowFor(entry, staged: false),
                        ],
                      ),
