...
DiffLine noNewline() => const DiffLine(
...
void main() {
  ...
  // The two cases below are the Dart suite's only departure from
  // SideBySideDiffTest.cpp's six, and the C++ suite gained the same pair in
  // the same commit -- `pairHunkForSideBySide`'s noNewlineMarker arm is the
  // one branch neither suite reached, and it is the arm most likely to be
  // "simplified" into `rows.add(row(line, line))` by someone reading the
  // context case just above it.

  test('a no-newline marker after a removal stays on the left side only', () {
    final DiffHunk hunk = hunkOf(<DiffLine>[
      context('keep'),
      removed('old'),
      noNewline(),
    ...
    // what it means.
    expect(rows[2].left!.kind, DiffLineKind.noNewlineMarker);
    expect(rows[2].right, isNull);
  });

  test(
    'a no-newline marker after an addition stays on the right side only',
    () {
      final DiffHunk hunk = hunkOf(<DiffLine>[
        context('keep'),
        removed('old'),
      ...
      expect(rows[1].right!.text, 'new');
      expect(rows[2].left, isNull);
      expect(rows[2].right!.kind, DiffLineKind.noNewlineMarker);
    },
  );
}
