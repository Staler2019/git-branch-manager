...
- 2026-09-05 — [fix/working-copy-unified-single-view](2026-09-05-fix-working-copy-unified-single-view.md) — `unified` 本來是兩個 view 上下疊（等於把兩欄轉九十度），改成一份依 index 區域排序的合併清單，方向改由卡片自己帶（U1 使用者裁定：對齊單位是 git 判斷出的區域變更，不是行號）；未追蹤檔案整份 stage 再整份 unstage 留在 Staged，量出反向 patch exit 0 但 index 條目留著空 blob，改走 `git restore --staged`；兩個「我自己寫錯」就地更正 —— `kind == Added` 是收窄不是不變式（mutation 回綠查出來的），U8 的落差只有邊框一個 token 而不是整個樣式沒實作
