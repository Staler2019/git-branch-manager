...
**Help was carrying a fifth item and this row did not notice**
(fix/macos-about-dialog-parity). `Check for updates…` sat between
`Report an issue` and `About`, labelled in source as "Not in the spec at
all". The count above still read `Help 4`, and so did
`gbm_menu_model.dart`'s own `// Help (4 items)` comment and
`gbm_action_id.dart`'s `// Help (4)` — three places asserting four while
the list held five, because nothing tested the Help menu's contents. The
item is now removed and `gbm_menu_model_test.dart` pins Help's four labels,
so the count is checked rather than asserted. `GbmActionId` is 65 values
after the removal; CLAUDE.md said 64 and had been stale before this round
too, so that number is now re-derived rather than carried forward. **This is a return to
conformance, not a deviation**: the update check keeps three entry points
that the spec has no opinion about (`about_dialog.dart`'s primary button,
Preferences → Advanced, and `AutoUpdateCheck` at startup), and About's is
the load-bearing one because `WelcomeScreen` renders no menu bar at all.

**Help → About was a platform fork, and this row could not see that either.**
`gbmMenus` is platform-neutral, so a row auditing the *model* says nothing
about what the item does: `PlatformMenuBarHost` listed `helpAbout` as
system-provided and appended a `PlatformProvidedMenuItem(about)`, so macOS
opened the native About panel while Windows and Linux opened
`AboutDialogContent`. Page 01's prose settles it — 「三平台統一樣式」 with the
Flutter-drawn scope pinned to 「視窗**內**所有內容」, and only the menu bar's
*position* in the platform-specific list. Fixed in the same branch; the
Apple menu keeps its own native About, which is where macOS convention puts
one and which `PlatformMenuBar` never touches.
...
| 05-C Remote-only / gone branch | ~~符合 (verify)~~ → **部分偏離（使用者裁定）** (fix/branch-prune-and-gone-marking) | `branch_tree_item.dart`, `features/context_menus/gbm_context_menus.dart` | **This row applied 05-C to rows spec never pointed it at, and that is what the user's bug report was about.** The old text is accurate as far as it goes — `_buildGoneMenuItems()` existed and rendered spec's Prune+Copy subset — but the *dispatch* in front of it read `_gone`, so **a local branch whose upstream had vanished got the remote-only menu**: no Checkout, no Merge, no Delete branch, just Prune and Copy. The branch was still sitting there on disk with commits in it. Spec 05-B is titled 「Local branch」 and 05-C 「Remote-only / gone branch」; a local branch with a gone upstream is the first, not the second, and the row's own 「gone 的列只留 Prune 與 Copy」 note is about the remote-only row. `_gone` no longer selects a menu at all — only `_isRemoteOnly` does. **Two user-ratified deviations from the catalog itself**, both recorded in `gbm_context_menus.dart`'s own doc comment because that file is the parity test's acceptance baseline: (1) `Prune this ref` is **deleted** — the ruling is that the word prune never appears in a menu, and fetch now prunes unclaimed refs in the background (see P02 item 12 below); (2) `Delete on remote…` is renamed `Delete remote branch…` and opens a dialog rather than deleting immediately. The group is 4 items. The enum value `remoteOnlyOrGoneBranch` keeps its old name — it is the parity test's key — with a comment saying why it outlived the concept. |
...
| Windows/Linux/macOS: native title bar reads `git-branch-manager` | ~~(not audited)~~ → **符合** (Tier 5, `0bf8971`) | `windows/runner/main.cpp:30`, `linux/runner/my_application.cc:48,52`, `macos/Runner/MainFlutterWindow.swift`, `macos/Runner/Base.lproj/MainMenu.xib:333` | **A real page-01 gap this audit missed entirely** while chasing the imaginary one above: all three mockup cards title the window `git-branch-manager`, but every platform still carried Flutter's scaffold default `gbm_flutter`. `lib/app.dart:14`'s `MaterialApp.title` does not reach the OS window title on desktop, so this could only be fixed in native runner code. **Stated honestly, so this row does not repeat the mistake above**: the title *text* is evidenced only by the mockups — no page-01 prose names it — so the reason to change it is that `gbm_flutter` is an unedited scaffold leftover while every other surface in the project (`MaterialApp.title`, the Info.plist usage descriptions, the README, the repo itself) already says `git-branch-manager`. `PRODUCT_NAME`/`BINARY_NAME` were deliberately left alone: they are also the built artifact names and `release.yml:161,204,226,254` hardcodes `gbm_flutter.app`/`.exe`. macOS additionally needs a deferred (next main-queue turn) re-assignment — measured, not defensive: a synchronous set in `awakeFromNib`, in the xib, or in `applicationDidFinishLaunching` is all reverted to `CFBundleName` before the window is on screen. Covered by `test/platform/window_title_test.dart`, which asserts the runner sources directly because no Dart test tier can reach them and PR CI never builds Windows at all. ~~**Still not addressed**: macOS's Apple-menu/About/Quit app name remains `gbm_flutter`~~ → **addressed** (fix/macos-about-dialog-parity, #67 closed): `macos/Runner/Info.plist`'s `CFBundleName` now holds the literal `git-branch-manager` instead of `$(PRODUCT_NAME)`, which is #67's candidate fix 1 — it decouples the *display* name from the *artifact* name, so `PRODUCT_NAME` stays `gbm_flutter` and `release.yml:161,204,226,254` keeps working untouched. Verified on a real `flutter build macos --debug`: the built bundle is still `gbm_flutter.app` with `CFBundleExecutable = gbm_flutter`, and its `Contents/Info.plist` reads `CFBundleName = git-branch-manager`. Still a different surface from the window title, and page 01's macOS card still does not draw the Apple menu — so this remains polish, not a conformance verdict. The CI hole that made the runner-source test necessary is tracked as #69 (it is one platform wider than stated above: PR CI runs no `flutter build macos` either). |

## Page 09 — Splitters (8 entries, `GbmSplitPane` + `tokens.dart`)

~~All 8 conform: default extent/ratio, minimum size, axis direction, and
double-click-to-reset all match the SPLITTERS table~~ → **6 符合, 2 使用者裁定的偏離**
(feat/working-copy-vertical-file-lists). Six still conform on every axis, backed by a
single `GbmSplitPane` widget with a `spec` parameter keyed to each `storageId`
(`main.sidebar`, `main.detail`, `main.files`, `main.log`, `cw.files`, `cw.panes`).
`main.log` defaults to collapsed/zero-extent as specified; `cw.panes` uses the
`[1, 1.12, 1]` flex ratio verbatim. Persistence goes through
`panelLayoutRepositoryProvider`, and `View → Reset panel sizes` clears it.

**The two Working Copy rows are overruled, and it is the `dir` column that goes.**
`SPLITTERS` reads `{ id: 'wc.columns', where: 'Unstaged ↔ Staged', dir: '垂直',
def: '1 : 1', min: '200px' }` and `{ id: 'wc.diff', where: '檔案區 ↔ Diff', dir: '水平',
def: '46 / 54', min: '150px' }`. `dir` is the *divider's own* direction, so those two
cells say「Unstaged and Staged sit side by side」and「the file area sits above the
diff」— which is what shipped, and what the user asked to be changed:
「目前左右不好看檔案內容」. Both dividers are now rotated: `wc.stack` (vertical
divider ⇒ Unstaged **above** Staged, flex `[1,1]`, min **96**) inside `wc.files`
(horizontal divider ⇒ file lists **left**, diff right, extent **260** / min **180**).
The storage ids changed with the axes, deliberately — [FLU-splitpane-axis-change] makes
re-keying an obligation, since `46/54` was a height ratio and `min: 200` a width floor,
and neither means anything on the other axis.

**Four of the five new numbers have no spec source and are recorded as such**
([SPEC-absent-not-faked]): extent 260, min 180, the stack's min 96, and the drop hint's
replacement wording 「拖曳檔案到下欄 = stage」 (spec's own string is 「拖曳檔案到右欄 =
stage」, which after the rotation names a column that is not there). Only the `[1,1]`
ratio survives from the spec's own cells. **96 is measured, not chosen**: bisected
against the board's own overflow guard at 85 red / 86 green, so the true floor is 86 and
the constant carries 10px of headroom; the exact-constant part of it is 78px of chrome
(header `rowHeightCompact` 26 + hint margin `space2` 8 + padding and borders 18 + one
26px file row). Full derivation and the two window widths it was drawn against:
`docs/claude-design-demo/working-copy-layout-spec.html`.

**符合 6 / 使用者裁定的偏離 2** — no orphaned or extra splitters found. Note this is a
*deviation*, not a gap: the rows were implemented, conformant, and then overruled, which
is [STRUCT-working-copy]'s checkbox situation exactly. Do not "fix" either row back.

---

## Page 02 — History (16 numbered items + the 〈Graph 連線規則〉 prose block)
...
| — | **〈Graph 連線規則〉 sentence 1: 「目前開發中的分支永遠佔 lane 0…其他分支一律往右配置」** | ~~never audited~~ → **符合** (fix/history-graph-commit-date-order) | `GraphBuilder.cpp` (`options_.trunkTip`), `LaneAllocator::reserve`, `HistoryProvider.h`'s `HistoryQuery::trunkTip`, `Session.cpp`'s `dispatchRefresh` | **This block was never audited at all** — this section's heading declares its unit as「16 numbered items」, and the prose blocks on the same page fall outside it; grepping all of `docs/reports/` for `lane 0` / `連線規則` / `永遠佔` returned nothing. `GraphBuilder.h`'s three invariants were already a sentence-by-sentence transcription of it and **the second had no implementation**: the first row emitted took lane 0, and the first row emitted is the newest tip, whichever branch that is. Lane 0 is now reserved before the first row and handed to HEAD's tip whenever it arrives, incoming edges included. **Recorded cost, user-ratified**: 「其他分支一律往右」 forbids lending the lane out, so lane 0 is blank for exactly as many rows as there are commits newer than HEAD's tip; History's new uncommitted-changes row fills that gap whenever the working copy is dirty. Three source comments claiming `historySeedRefs()`'s argument order pinned the trunk leftmost were measured false under both orderings and corrected. See [SPEC-lane-zero-is-head] and [SPEC-audit-unit-is-not-the-page]. |
| — | **〈Graph 連線規則〉 sentences 2–4 (貝茲曲線接進 lane 0, 支線側轉折, 主線不歪)** | 符合 | `graph_edge_geometry.dart`, `graph_column_painter.dart` | Unchanged by this round and now checked rather than assumed: `patchIncoming()` plus the release loop bend every arriving edge into the claimed lane, and the claimed lane never moves for them. |
| — | **Row order follows the Date column** | ~~不符合~~ → **符合** (fix/history-graph-commit-date-order) | `HistoryProvider.cpp`'s `toRevListArgs()` | Not a numbered item either, but a plain contradiction between two things P02 does specify: the Date column renders `row.commitTime`, and the walk was ordered by `--topo-order`, which finishes one branch before starting the next. Measured on this repository: **15 time inversions over 835 rows, 7 of them on merge rows** — the reported 「merge 全擠在最上方」. Now `--date-order`; see [GIT-topo-order-unconditional]. |

**15/16 符合, 1 real rendering gap (item 6's chip-merge rule)**, plus three previously-unaudited rows added above. Item 2's original 符合 was withdrawn and re-earned by feat/p02-action-toolbar; the count is unchanged because the row was already counted as passing when it was not. **The same is true of item 13** _(feat/p03-working-copy-redesign)_: the tabs conformed, their row's placement did not, and the count never moved because the cell had only ever been asked about the tabs. Three P02 rows have now passed for the wrong half (2, 12, 13) — when re-auditing this page, check the row's *title* against the spec's own wording before reading its evidence.
...
| 2, 4 — Splitters | ~~符合~~ → **使用者裁定的偏離** (feat/working-copy-vertical-file-lists) | `tokens.dart` `splitterWcFiles` / `splitterWcStack`, `working_copy_view.dart`, `working_copy_board.dart` | ~~1:1 and 46/54 match `SPLITTERS`. Unchanged, re-checked.~~ **Both dividers are rotated 90° on the user's ruling** — the file lists are a vertical stack (Unstaged above Staged) inside a fixed-width **left** column, with the diff filling the right. `46/54` is gone entirely (it was a height ratio); `1:1` survives as the stack's ratio. See the Page 09 section for the two overruled `SPLITTERS` cells, the four numbers with no spec source, and why the storage keys changed with the axes. |
...
| 8 — diff area | 符合 | `working_copy_diff_pane.dart`, `scoped_diff_view.dart` | **符合 was true of every file the pane could answer for, and an untracked file was not one of them** _(claude/working-copy-untracked-files-qq2gnc, after a user report that 「working copy cannot view untracked file in 2 file era or unified era as unstaged」)_: `git diff` prints nothing for a path in neither the index nor HEAD, so the reply arrived as an ordinary empty diff and both modes drew 「Nothing unstaged」 next to a row badge reading `+12`. Both modes, because `_side()` is shared — the mode switch was never the variable. The same wall made `PartialStageOperation` and `DiscardLinesOperation` answer 「No pending changes found」, so the granularities this page's other row calls 符合 were unreachable for untracked files too. The core answers them with `git diff --no-index` now ([GIT-no-index-sees-untracked]) and the rebuilt patch carries a `new file mode` header ([GIT-new-file-patch-needs-dev-null]). A diff refused for its size is also no longer drawn as 「nothing」 ([CPP-parse-refuses-over-cap]). Two modes (`2 file` / `unified`). Staging is by **scope** — changes separated by ≤2 unchanged lines merge, never across a hunk — with a button at the end of each run, plus a one-shot temporary scope from an ordinary text selection. The per-line checkbox that used to serve this is gone from `DiffPage`, which is now read-only. In `2 file` mode the two sides are split by a real `GbmSplitPane` (`wc.diffSides`), added in C18: spec page 09's `SPLITTERS` table has no row for it because that table predates 變體 B's two-sided diff, so the spec is *extended* here rather than deviated from — the numbers followed `wc.columns`, the same view's other 1:1 split (that id is `wc.stack` since feat/working-copy-vertical-file-lists; `wc.diffSides` kept both its axis and its numbers, and is the one Working Copy divider that round did not touch). **The default mode is `unified` as of the same round**, so the two-sided split is no longer what opens — the pane now sits in the right half of the view, and splitting *that* into two monospace columns by default would have reproduced the complaint the rotation was for. **The one-shot scope's placement was wrong until a user pointed at it** _(after C18)_: the style demo nests `.variant-B-temp` **inside** `.variant-B-card`, wrapping the selected rows in place while the card goes `.variant-B-card-muted` and its own button `.variant-B-btn-off`; what shipped was an extra card pinned to the top of the column. That was an implementation convenience — a fixed slot cannot reparent the keyed rows below it — and the hazard it avoided is answered by the rows' own `GlobalKey`s, which move the element rather than rebuild it. **A verdict of 符合 on 「a one-shot temporary scope exists」 could not see this**: the cell named the capability, and the demo's DOM is where the placement lives. |
| 9 — tab badge = deduped unstaged+staged count | 符合 | `WorkingCopyStatus.entries` | One entry per path already. |
| 10 — List/Tree toggle: ONE shared preference across Working Copy / History / Compare / Conflict window | ~~缺少 (partial)~~ → **符合** | `changed_files_panel.dart:187`, `compare_page.dart:334`, `conflict_resolve_window.dart:524`, `panel_file_diff_detail.dart:73`, `working_copy_view.dart:141` | **This row was stale, not wrong when written.** It said 「only `working_copy_view.dart:104` actually reads the provider … History, Compare, and the Conflict window never read this provider」. All four now `ref.watch(fileListViewModeProvider)`; the fix landed in an earlier round and this cell was never revisited. Re-verified by grepping every reader, not by sampling. The toggle itself is now `GbmSegmentedControl`, shared with the diff area's mode switch. **And that verdict covers one clause of item 10 out of five** _(feat/working-copy-vertical-file-lists)_: this row's title says 「ONE shared preference across …」, which is item 10's *last* sentence, and the audit unit never included its first two. Quoted in full, since the truncation is what let this sit: 「每個檔案清單標題右側一組兩鍵切換：**平鋪完整路徑，或依資料夾摺成樹狀。樹狀模式下只有一個子項的資料夾會自動串接成 `lib/app/views` 一列，不會逐層縮排浪費寬度。**資料夾列的 checkbox 是三態…收合狀態與模式各清單獨立記憶。同一個設定套用到 Working Copy 兩欄、History 的 Changed files、Compare 的 Files、以及 Conflict 視窗的檔案清單。」 **Both bolded clauses were broken on all six surfaces at once.** `FileListModeSwitcher.leafBuilder` took no label, so every call site drew the item's full path — tree mode nested the row under a folder row and then spelled the prefix again on the leaf, making 「摺成樹狀」 cost indentation and buy nothing; and `FileTree._collapseIfSingleChild` did `leafPath!.split('/').last` when a collapsed chain ended in a **file**, discarding the very concatenated prefix the spec's own `lib/app/views` example is about (the directory branch was already correct, which is why every fixture whose chain ended in a folder looked right). Fixed in that round; the contract is pinned as [STRUCT-leaf-label-from-switcher]. **Corrected the next day (2026-09-06), 使用者裁定**: ~~that round's fix concatenated the chain onto the *file* as well~~ — 「樹狀模式下，我想要的是像 vscode 一樣，folder 可以堆疊名稱，但是檔案不會有 folder」, reported against `docs/ledger/…md` + `docs/rules/…md` drawing as two prefixed file rows with no folder row at all. The collapse now stops at the file, so this clause's verdict rests on the **folder** chain only, which is what the spec's `lib/app/views` example actually shows; the file half was this repo's extension of it and is gone. Item 10's own 「不會逐層縮排浪費寬度」 is still honoured by the folder chain _(fix/working-copy-unified-single-view 追加三)_. **The tell is this row's own evidence column** — five `ref.watch` call sites, i.e. a capability (「the preference is shared」) standing in for the surface (「tree mode folds」), which is [SPEC-cell-names-capability] and, because the clause sat outside the heading's stated unit, [SPEC-audit-unit-is-not-the-page] as well. Note also that item 10 **contradicts itself** on 模式: 「收合狀態與**模式**各清單獨立記憶」 against 「**同一個設定**套用到 …」 four surfaces. The code implements the second (one shared provider) and this row has always ratified that reading; collapse state genuinely is per-list (`FileTreeList`'s own `_expandedFolders`). Recorded, not resolved — nothing in the round turned on it, and it needs a ruling rather than an implementer picking a side. |

**10/10 items accounted for**: ~~6 符合~~ → **4 符合**, ~~3~~ → **4 刻意偏離（使用者裁定）**
— items 1/3/10's checkbox, plus items 2/4's two rotated splitters
(feat/working-copy-vertical-file-lists) — and **2 符合-corrected**: item 10's shared
preference, which was stale, and item 10's 「平鋪／摺成樹狀」 clauses, which were
**broken on all six surfaces** and outside the audit unit this row's heading declared.
Zero open gaps on this page; a deviation is not a gap and is not counted as one, and
the two corrections were real defects that are now fixed rather than reclassified.
One item-10 clause is left **unresolved and flagged, not decided**: the spec
contradicts itself on whether the List/Tree *mode* is per-list or shared, and picking
a side is a ruling, not an implementation detail.
