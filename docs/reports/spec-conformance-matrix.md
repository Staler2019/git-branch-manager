...
| 05-E Commit | **缺少** | `commit_row.dart` menu build | Renders: Checkout this commit / Cherry-pick / Create branch here… / Copy SHA / Revert commit (5 items, no submenu). Missing vs. spec's 7 top-level + "More actions" submenu of 5: `Merge into current`, `Compare with…`, and the ENTIRE submenu (`Rebase onto here`, `Reset branch to here…`, `Revert commit`, `Export as patch…`, `Compare with working copy`). Note spec keeps `Revert commit` inside the submenu; code promotes it to top level. Classification **(i)** for all — `gbm_merge_branch`/`gbm_request_compare_refs`/`gbm_rebase_start`/`gbm_reset_to`/`gbm_revert`/`gbm_patch_export`/`gbm_request_compare_with_working_copy` all exist. |
...
| 05-K Commit file | **缺少** | `changed_files_panel.dart` menu build | Renders: View diff in this commit / File history / Blame at this commit / Copy path / More actions → Restore file to this state (5 top-level + 1 submenu item). Missing vs. spec's 7 top-level + 4-item submenu: `Compare with working copy`, `Open file at this revision`, `Open terminal here` (top level); `Restore and stage`, `Save this revision as…`, `Export as patch…` (submenu). Classification: `Compare with working copy`/`Open terminal here` are **(i)**; `Open file at this revision`/`Save this revision as…` are **(ii)** — no blob-read capi entry point exists (`gbm_capi.h` has no `show`/`cat-file`/blob-read function, confirmed by exhaustive grep of its 109 exported functions). |
