...
| 5 | **Memory margin is thinner than it looks** | 124.2 bytes/commit on real vscode — matches the synthetic fixture almost exactly (124.2 vs 124.0) — projecting to 59.2 MB at 500k against a **64 MB target: 7% headroom**. | Fixed per-row struct cost in `GraphSnapshot`, consistent across synthetic and real data — structural, not a topology artifact. | **Since fixed:** `GraphSnapshot::index_` (`std::unordered_map<ObjectId, RowId>`, ~43% of the total by the estimator's own accounting, with an uncounted bucket array on top) had zero callers of its public accessor and was replaced with `oidOrder_`, a `std::vector<RowId>` of row indices sorted by oid, with `findRow()` binary-searching it instead of hashing. `approximateBytes()` now counts the replacement's exact capacity instead of estimating a hash-map node, so the metric got more honest as well as smaller. Measured 75-77 bytes/commit post-fix across three different fixture topologies (5k synthetic, 20k perf-shaped, this repo's own history), consistent with this row's "structural, not topology" finding. Both hardcoded 140-byte budget checks were tightened to 92 (~18% above measured). Per this row's own recommendation, the ratio is now tracked in CI: `gbm_graph_check` prints a machine-readable `graph-memory:` line and the nightly `commit-graph-ratio` job publishes it to the run's Step Summary. See `docs/PERFORMANCE.md`'s "GraphSnapshot memory layout" section. |
| 6 | **`for-each-ref` scales with ref count, not commit count** | 25–49ms on this repo (only 30 refs after the fork clone) — but `docs/PERFORMANCE.md`'s own 3,798-ref fixture measured 0.8–2.2s for *one* call, more than the entire 50k-commit commit-graph-accelerated walk. Not reproducible here since this vscode fork only carries 30 refs. | `%(upstream:track)` per-ref cost (doc: ~440ms refname-only vs ~770ms full format, same repo). | Duplicate-load already fixed (`refreshRefsAndHistory()`). **Since fixed:** the remaining, previously-unapplied mitigation this row named — `core/workers/Debouncer.h` existed but was unused — is now wired. `RepositorySession` routes `refreshRefs()`/`refreshHistory()`/`refreshRefsAndHistory()` through `RefreshCoalescer` (`core/workers/RefreshCoalescer.h`, pure and unit-tested), a 150ms trailing window (matching `MainWindow`'s existing debounce interval) that merges a burst into one `for-each-ref` load as the union of what was asked for, with `Explicit` always outranking a silent `AutoFetchResync` in the merge. A request arriving mid-refresh folds in and produces exactly one follow-up rather than queueing or being dropped. `setHistoryFilter()` deliberately bypasses this — a filter change must be seen immediately, not delayed behind a coalescing window. The window's cost is its own `coalesce_ms` segment in the `GBM_TIMING=1` probe (bottleneck #4) rather than silently inflating `queue_ms`. See `docs/PERFORMANCE.md`'s "Refresh coalescing" section for the full design and the same honest limitation bottlenecks #2/#3/#4's fixes already state: `RepositorySession` itself still has no test harness, so only the pure decision table (`RefreshCoalescer`, `coalesce_ms` formatting) is regression-tested — the Qt wiring was verified manually. |
