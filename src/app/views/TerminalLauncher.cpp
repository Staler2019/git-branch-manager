...
bool launchMacTerminal(const QString& path, QString* error) {
    ...
    if (QProcess::startDetached(QStringLiteral("open"), {QStringLiteral("-a"),
                                                         QStringLiteral("Terminal"), path})) {
        return true;
    }
    if (error != nullptr) {
...
bool launchWindowsTerminal(const QString& path, QString* error) {
    ...
    if (QProcess::startDetached(QStringLiteral("cmd.exe"),
                               {QStringLiteral("/c"), QStringLiteral("start"), QStringLiteral(""),
                                QStringLiteral("/d"), path, QStringLiteral("cmd.exe")})) {
        return true;
    }
    if (error != nullptr) {
...
bool launchLinuxTerminal(const QString& path, QString* error) {
    ...
    const QString envTerminal = QProcessEnvironment::systemEnvironment().value(
    ...
    for (const QString& candidate : candidates) {
        ...
        const QString command = candidate == QStringLiteral("xterm")
                                   ? QStringLiteral("cd %1 && exec xterm").arg(path)
                                   : QStringLiteral("%1 --working-directory=%2")
                                         .arg(candidate, path);
