...
GBM_API int32_t gbm_submodules_json(GbmSessionHandle session) {
    ...
    if (!submodules) {
        setStagingBuffer(toJson(GitError(GitError::Code::NotFound, "no submodule list published yet")));
...
GBM_API void gbm_submodule_add(GbmSessionHandle session, const char* url, const char* path, const char* branch) {
...
GBM_API void gbm_submodule_init(GbmSessionHandle session, const char* const* paths, int32_t pathCount, int32_t recursive) {
...
GBM_API void gbm_submodule_sync(GbmSessionHandle session, const char* const* paths, int32_t pathCount, int32_t recursive) {
    SubmodulePathsRequest request;
    request.paths = toStringVector(paths, pathCount);
    request.recursive = recursive != 0;
...
GBM_API void gbm_submodule_deinit(GbmSessionHandle session, const char* const* paths, int32_t pathCount, int32_t force) {
