// Integration tests for gbm_request_working_tree_content() and
// gbm_parse_conflict_markers() -- the conflict-resolve-editor data path
// (gap #53: rich conflict resolution editor). Reuses MergeApiTest's real
// two-branch conflict setup so the on-disk file actually contains git's own
// conflict markers, not a hand-written approximation of them.
#include "capi/gbm_capi.h"
#include "core/git/GitExecutable.h"
#include "support/GitCli.h"

#include <chrono>
#include <condition_variable>
#include <cstdlib>
#include <filesystem>
#include <fstream>
#include <functional>
#include <gtest/gtest.h>
#include <mutex>
#include <string>
#include <vector>

namespace gbm::capi {
namespace {

using ::gbm::testing::GitCli;

struct EventLog {
    std::mutex mutex;
    std::condition_variable cv;
    std::vector<std::pair<int32_t, std::string>> events;

    void add(int32_t eventType, std::string payload) {
        std::lock_guard<std::mutex> lock(mutex);
        events.emplace_back(eventType, std::move(payload));
        cv.notify_all();
    }

    bool waitFor(const std::function<bool(const std::vector<std::pair<int32_t, std::string>>&)>& pred,
                std::chrono::milliseconds timeout = std::chrono::seconds(10)) {
        std::unique_lock<std::mutex> lock(mutex);
        return cv.wait_for(lock, timeout, [&] { return pred(events); });
    }

    std::string lastPayloadOfType(int32_t type) {
        std::lock_guard<std::mutex> lock(mutex);
        std::string last;
        for (const auto& [eventType, payload] : events) {
            if (eventType == type) last = payload;
        }
        return last;
    }
};

void logCallback(GbmSessionHandle, int32_t eventType, const uint8_t* payload, int32_t payloadLen, void* userData) {
    auto* log = static_cast<EventLog*>(userData);
    std::string body;
    if (payload != nullptr) {
        body.assign(reinterpret_cast<const char*>(payload), static_cast<std::size_t>(payloadLen));
        gbm_free_event_payload(payload);
    }
    log->add(eventType, std::move(body));
}

class ConflictMarkerApiTest : public ::testing::Test {
protected:
    static void SetUpTestSuite() {
        auto detected = GitExecutable::detect();
        if (!detected) {
            GTEST_SKIP() << "no usable git found: " << detected.error().message;
        }
    }

    void SetUp() override {
        const auto* info = ::testing::UnitTest::GetInstance()->current_test_info();
        repo_ = std::filesystem::temp_directory_path() / ("gbm-capi-conflict-marker-" + std::string(info->name()));
        std::filesystem::remove_all(repo_);
        std::filesystem::create_directories(repo_);

        ASSERT_EQ(runGit({"init", "--quiet", "--initial-branch=main"}), 0);
        ASSERT_EQ(runGit({"config", "user.email", "test@example.invalid"}), 0);
        ASSERT_EQ(runGit({"config", "user.name", "Test"}), 0);
        ASSERT_EQ(runGit({"config", "commit.gpgsign", "false"}), 0);

        std::ofstream(repo_ / "shared.txt") << "base\n";
        ASSERT_EQ(runGit({"add", "shared.txt"}), 0);
        ASSERT_EQ(runGit({"commit", "--quiet", "-m", "Base commit"}), 0);

        ASSERT_EQ(runGit({"checkout", "--quiet", "-b", "feature"}), 0);
        std::ofstream(repo_ / "shared.txt") << "feature change\n";
        ASSERT_EQ(runGit({"add", "shared.txt"}), 0);
        ASSERT_EQ(runGit({"commit", "--quiet", "-m", "Feature edits shared.txt"}), 0);
        ASSERT_EQ(runGit({"checkout", "--quiet", "main"}), 0);
        std::ofstream(repo_ / "shared.txt") << "main change\n";
        ASSERT_EQ(runGit({"add", "shared.txt"}), 0);
        ASSERT_EQ(runGit({"commit", "--quiet", "-m", "Main edits shared.txt"}), 0);
        // Leaves shared.txt on disk with real <<<<<<</=======/>>>>>>> markers.
        runGit({"merge", "--quiet", "feature"});

        session_ = gbm_session_open(repo_.string().c_str(), (repo_ / ".git").string().c_str(), "");
        ASSERT_NE(session_, nullptr);
        gbm_register_callback(session_, &logCallback, &log_);
    }

    void TearDown() override {
        if (session_ != nullptr) {
            gbm_session_close(session_);
        }
        std::error_code ec;
        std::filesystem::remove_all(repo_, ec);
    }

    /// Fixture git, without a shell in the middle -- see tests/support/GitCli.h
    /// for why that matters (one process instead of two, and no per-platform
    /// quoting hazard).
    int runGit(std::vector<std::string> args) {
        return GitCli::run(repo_, std::move(args));
    }

    std::filesystem::path repo_;
    GbmSessionHandle session_ = nullptr;
    EventLog log_;
};

TEST_F(ConflictMarkerApiTest, RequestWorkingTreeContentReadsTheConflictedFileWithMarkers) {
    gbm_request_working_tree_content(session_, "shared.txt");
    ASSERT_TRUE(log_.waitFor([](const auto& events) {
        for (const auto& [type, payload] : events) {
            if (type == GBM_EVENT_WORKING_TREE_CONTENT_READY) return true;
        }
        return false;
    }));

    const std::string reply = log_.lastPayloadOfType(GBM_EVENT_WORKING_TREE_CONTENT_READY);
    EXPECT_NE(reply.find("\"path\":\"shared.txt\""), std::string::npos) << reply;
    EXPECT_NE(reply.find("\"editable\":true"), std::string::npos) << reply;
    EXPECT_NE(reply.find("<<<<<<<"), std::string::npos) << reply;
    EXPECT_NE(reply.find("feature change"), std::string::npos) << reply;
    EXPECT_NE(reply.find("main change"), std::string::npos) << reply;
}

TEST_F(ConflictMarkerApiTest, RequestWorkingTreeContentReportsNotEditableForAMissingPath) {
    gbm_request_working_tree_content(session_, "does-not-exist.txt");
    ASSERT_TRUE(log_.waitFor([](const auto& events) {
        for (const auto& [type, payload] : events) {
            if (type == GBM_EVENT_WORKING_TREE_CONTENT_READY) return true;
        }
        return false;
    }));

    const std::string reply = log_.lastPayloadOfType(GBM_EVENT_WORKING_TREE_CONTENT_READY);
    EXPECT_NE(reply.find("\"editable\":false"), std::string::npos) << reply;
    EXPECT_NE(reply.find("\"content\":\"\""), std::string::npos) << reply;
}

TEST_F(ConflictMarkerApiTest, ParseConflictMarkersSplitsOneRegionIntoOursAndTheirs) {
    gbm_request_working_tree_content(session_, "shared.txt");
    ASSERT_TRUE(log_.waitFor([](const auto& events) {
        for (const auto& [type, payload] : events) {
            if (type == GBM_EVENT_WORKING_TREE_CONTENT_READY) return true;
        }
        return false;
    }));
    const std::string reply = log_.lastPayloadOfType(GBM_EVENT_WORKING_TREE_CONTENT_READY);

    // Pull the raw file straight off disk rather than JSON-decoding the
    // event payload here -- this test is about gbm_parse_conflict_markers,
    // not about re-implementing a JSON parser.
    std::ifstream file(repo_ / "shared.txt");
    const std::string content((std::istreambuf_iterator<char>(file)), std::istreambuf_iterator<char>());
    ASSERT_NE(content.find("<<<<<<<"), std::string::npos);

    ASSERT_EQ(gbm_parse_conflict_markers(content.c_str()), 0);
    std::string json(static_cast<std::size_t>(gbm_last_result_json_len()), '\0');
    gbm_last_result_json_copy(reinterpret_cast<uint8_t*>(json.data()), static_cast<int32_t>(json.size()));

    EXPECT_NE(json.find("\"regionCount\":1"), std::string::npos) << json;
    EXPECT_NE(json.find("\"wellFormed\":true"), std::string::npos) << json;
    EXPECT_NE(json.find("main change"), std::string::npos) << json;
    EXPECT_NE(json.find("feature change"), std::string::npos) << json;
    // No literal marker line ever appears in a segment's text.
    EXPECT_EQ(json.find("<<<<<<<"), std::string::npos) << json;

    (void)reply;
}

TEST_F(ConflictMarkerApiTest, ParseConflictMarkersOnPlainTextReportsNoRegions) {
    ASSERT_EQ(gbm_parse_conflict_markers("just a normal file\nwith two lines\n"), 0);
    std::string json(static_cast<std::size_t>(gbm_last_result_json_len()), '\0');
    gbm_last_result_json_copy(reinterpret_cast<uint8_t*>(json.data()), static_cast<int32_t>(json.size()));

    EXPECT_NE(json.find("\"regionCount\":0"), std::string::npos) << json;
    EXPECT_NE(json.find("\"wellFormed\":true"), std::string::npos) << json;
}

}  // namespace
}  // namespace gbm::capi
