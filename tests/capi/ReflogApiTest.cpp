// Integration test for the M6 reflog slice of the extern "C" surface
// (gbm_capi.h).
...
protected:
    ...
    void SetUp() override {
        ...
        repo_ = std::filesystem::temp_directory_path() / ("gbm-capi-reflog-" + std::string(info->name()));
...
TEST_F(ReflogApiTest, ReflogListsHeadMovementsNewestFirst) {
    gbm_request_reflog(session_, "");

    ASSERT_TRUE(log_.waitFor([](const auto& events) {
        for (const auto& [type, payload] : events) {
            if (type == GBM_EVENT_REFLOG_READY) return true;
        }
        return false;
    }));

    const std::string payload = log_.lastPayloadOfType(GBM_EVENT_REFLOG_READY);
    EXPECT_NE(payload.find("\"index\":0"), std::string::npos) << payload;
    EXPECT_NE(payload.find("Second commit"), std::string::npos) << payload;
    EXPECT_NE(payload.find("First commit"), std::string::npos) << payload;
}
